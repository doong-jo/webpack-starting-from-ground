...
Webpack은 웹 어플리케이션을 개발할 때 가장 인기 있는 번들러입니다. React 앱을 빌드해주는 도구로 치부하고 CRA를 그대로 사용하기 보다는 이것이 어떻게 동작하는지, 핵심 원리는 무엇인지 이해해야 하지 않을까? 라는 생각에서 글을 적게되었습니다. 하지만 Webpack에서 사용하는 모든 플러그인과 번들러, 최적화 옵션을 공부하기 보다는 Webpack의 동작원리와 핵심 요소들을 이해하고 상황에 맞게 필요한 것들을 학습하는 것이 효율적이라고 생각합니다. 그래서 **이 글에서는 Webpack의 깊이있는 내용보다 얕게 '이것만은 알아야 하지 않을까' 라고 생각되는 내용을 적어봤습니다.**
