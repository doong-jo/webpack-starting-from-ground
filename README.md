...
따라서 **Webpack을 사용하여 개발자가 직접 하기 어려운 위와 같은 이슈들(성능 및 스크립트 로드 시간)을 관리할 수 있습니다. 또한 강력한 기능 중 하나가 코드 스플리팅인데 코드를 다양한 번들(chunk)로 나눠서 필요한 번들만 로드하거나 병렬로 로드함으로 인해 필요한 코드(스크립트)만 번들링 할 수 있습니다.**
